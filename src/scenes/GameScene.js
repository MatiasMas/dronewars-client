...
export class GameScene extends Phaser.Scene {
    ...
    playerScore = 0;
    static SCORE_DRONE = 10;
    static SCORE_CARRIER = 50;
    nombreJugador = "";
    textoSiglas = null;
    ...
    panelResultado = null;
    timerPortadronesTexto = null;
    timerPortadronesDeadlineMs = null;
    timerPortadronesActivo = false;
    timerPortadronesObjetivoTexto = '';
    avisoPausaContenedor = null;
    sidebarUnidades = [];
    sidebarUnidadesPorId = new Map();
    sidebarMezclaAlertaPorUnidad = new Map();
    sidebarElementos = [];
    sidebarUnidadSeleccionadaId = null;
    sidebarNecesitaRender = true;
    tweenCamaraSeleccion = null;
    ...
    static PANEL_DRONES_RATIO_ANCHO = 0.2;
    static PANEL_LATERAL_RATIO_ALTO = 0.2;
    static SIDEBAR_MARGEN_INTERNO = 12;
    static SIDEBAR_ALTO_CABECERA = 48;
    static SIDEBAR_COLUMNAS = 2;
    static SIDEBAR_ESPACIO_GRID = 8;
    static SIDEBAR_LADO_CUADRO_BASE = 92;
    static SIDEBAR_COLOR_FONDO_NORMAL = 0x121a2c;
    static SIDEBAR_COLOR_FONDO_ALERTA = 0x2a1a10;
    static SIDEBAR_COLOR_FONDO_DESTRUIDA = 0x505050;
    static SIDEBAR_COLOR_BORDE_NORMAL = 0x2f4d8c;
    static SIDEBAR_COLOR_BORDE_ALERTA = 0xff8c00;
    static SIDEBAR_COLOR_BORDE_DESTRUIDA = 0xb5b5b5;
    static SIDEBAR_COLOR_DETALLE_NORMAL = 0x93c5fd;
    static SIDEBAR_COLOR_DETALLE_ALERTA = 0xffb347;
    static SIDEBAR_COLOR_DETALLE_DESTRUIDA = 0xd0d0d0;
    static SIDEBAR_FACTOR_LERP_ALERTA = 0.012;
    static MOVE_STEP = 5;
    ...
    static CUENTA_REGRESIVA_PORTADRONES_MS = 2 * 60 * 1000;
    //Visibilidad
    ...
    async create(data = {}) {
        ...
        const preferredPlayerId = (data.preferredPlayerId ?? preferredFromRegistry);
    ...
    centrarCamaraConPanelIzquierdo(objetivoX, objetivoY) {
        const { scrollX, scrollY } = this.calcularScrollCamaraConPanelIzquierdo(objetivoX, objetivoY);
        this.cameras.main.setScroll(scrollX, scrollY);
    }
    calcularScrollCamaraConPanelIzquierdo(objetivoX, objetivoY) {
    ...
    animarCamaraConPanelIzquierdo(objetivoX, objetivoY) {
        const camara = this.cameras.main;
        const { scrollX, scrollY } = this.calcularScrollCamaraConPanelIzquierdo(objetivoX, objetivoY);
    ...
    selectPreferredOrFirstAvailablePlayer(preferredPlayerId) {
        if (preferredPlayerId) {
            const preferred = this.availablePlayers.find(p => p.playerId === preferredPlayerId && p.available);
            if (preferred) {
    ...
    */
    setupEventListeners() {
        ...
        this.selectionManager.on(ClientInternalEvents.SELECTION_CONFIRMED, (unit) => {
            this.updateSelectedUnitCoordsText();
            this.emitirSeleccionParaSidebar(unit.unitId);
        });
    ...
    seleccionarUnidadDesdeSidebar(unitId) {
    ...
    ciclarSeleccionUnidades(direccion) {
    ...
    emitirSeleccionParaSidebar(unitId) {
    ...
    drawUI() {
        ...
        this.timerPortadronesTexto = this.add.text(this.cameras.main.centerX, 58, '', { fontSize: '18px', color: '#ffd166', fontStyle: 'bold', align: 'center' }).setOrigin(0.5).setScrollFactor(0).setDepth(102).setVisible(false);
    ...
    setupPointerControls() {
        ...
        this.input.on('pointerdown', (pointer, gameObjects) => {
            if (this.menuPausaVisible) {
                if (pointer.leftButtonDown()) {
                    this.manejarClickMenuPausa(pointer.x - this.cameras.main.x, pointer.y - this.cameras.main.y);
                }
                return;
            }
            if (this.partidaPausada || this.menuPausaVisible || this.partidaFinalizada) {
    ...
    manejarIngresoNombre(event) {
        if (!this.capturandoNombre)
            return;
        ...
        if (this.nombreJugador.length === 3) {
            ...
            this.highScoreManager.saveScore(this.nombreJugador, this.playerScore);
    ...
    iniciarTimerPortadrones(portadronesPropioDestruido) {
    ...
    actualizarTimerPortadrones() {
        ...
        this.timerPortadronesTexto.setText(`${this.timerPortadronesObjetivoTexto} ${reloj} para destruir el portadrones contrario.`);
        if (restanteMs <= 0) {
            this.timerPortadronesActivo = false;
        }
    }
    limpiarSidebarElementos() {
    ...
    actualizarSidebarUnidades(unidadesActualizadas) {
        const idsPropiosActualizados = new Set();
    ...
    calcularGridSidebar(cantidadUnidades, anchoPanel, altoPanel) {
    ...
    obtenerUnidadesSidebarOrdenadas() {
    ...
    obtenerEtiquetasSidebarPorUnidad(unidades) {
    ...
    lerpColorEnteroSidebar(colorDesde, colorHasta, factor) {
    ...
    colorNumeroAHexSidebar(color) {
        return `#${color.toString(16).padStart(6, '0')}`;
    }
    esUnidadRenderizableSidebar(tipo) {
        return tipo === 'AERIAL_DRONE'
            || tipo === 'NAVAL_DRONE'
            || tipo === 'AERIAL_CARRIER'
            || tipo === 'NAVAL_CARRIER';
    }
    esPortadronSidebar(tipo) {
        return tipo === 'AERIAL_CARRIER' || tipo === 'NAVAL_CARRIER';
    }
    obtenerPrioridadSidebar(tipo) {
        switch (tipo) {
            case 'AERIAL_CARRIER':
                return 0;
            case 'NAVAL_CARRIER':
    ...
    obtenerUnidadesConEnemigoEnVision() {
        const unidadesConVision = new Set();
        const enemigosDron = Array.from(this.knownUnits.values()).filter(unidad => !this.playerUnitIds.has(unidad.unitId) &&
